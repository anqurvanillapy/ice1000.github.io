...
这是一个 "条件" ，这个 "条件" 是两个其他条件的组合，它拿到两个条件变成一个新条件，
对应上面说的 "不是由 $ \rightarrow $ 结合成的类型" 。
...
（顺带一提，类型的阶（顺带一提， Agda 中表示类型的阶的类型正好是 `Level` ，中文意思就有阶的意思，
