...
首先我们需要打开MPS，并像上次一样创建一个新工程。
这次我们选择创建一个Language工程，并勾选**Create a SandBox Solution**选项。

![](https://coding.net/u/ice1000/p/Images/git/raw/master/blog-img/mps/1/0.png)

这时，你会进入到你自己创建的工程界面。左边有一个不同于其他JB IDE的**logical view**，它是这个工程的逻辑结构。

打开文件系统中的MPS工程，你会看到一大堆：
...
将代码托管到Git时，一般不上传java和class文件（这些文件一般都是MPS的Code Generator生成的，不是“源代码”性质的源代码）。

mpl是整个MPS工程的配置文件（可以理解为： MPs Language的缩写），而mps是MPS工程组件文件，其实就是xml。

好，我们回到工程界面。

![](https://coding.net/u/ice1000/p/Images/git/raw/master/blog-img/mps/1/1.png)

这时我们来理一下LOP的基本思想。其实LOP和OOP、POP多有几分相似之处，他们都是“为了实现代码复用”而做出的一种“将代码模块化”的方式。
...
注意一下，所谓的“函数式编程（FP）”不属于这个分类范畴，这里不做讨论。POP不是FP，POP是有很多（大部分情况下）函数，
但是 `有函数 != 函数式编程` 请务必注意！
...
上文摘自[Wikipedia中文区](https://zh.wikipedia.org/zh-hans/%E8%BF%87%E7%A8%8B%E5%BC%8F%E7%BC%96%E7%A8%8B)
...
那么在LOP里面，“语言”这个概念也被泛化了——所有的*语言*都是DSL，所有的*语言*之间都是可以交互的，你可以把很多门*语言*混着写。
...
下文的*语言*除非特别注明，都是指LOP中的*语言*，为了方便，我不再使用斜体标注。

## MPS中的LOP

MPS中，语言是由很多**Concept**组成的，这些**Concept**有各自的属性、限制等，它们相当于是**AST中的节点**。

由于现在的编程模式大多数都是*写代码*，于是MPS也把程序编辑的过程弄成了*一种类似写代码*的形式，叫**破界神编辑**，上文已经说过啦。

因此，我们需要定义语言的*语法*。语言的语法，在MPS里面叫做**Editor**。

我们通过MPS创建的语言所编写的代码，需要通过**Generator**生成目标代码（比如本文使用的Java）。
...
然后按Dependency选项卡下的绿色加号的导入JDK这个Dependency：

![](https://coding.net/u/ice1000/p/Images/git/raw/master/blog-img/mps/1/8.png)

### Concept

我们首先创建我们需要的Concept——PrintlnSet，和Println。
...
+ 不能是AST根节点
+ 拥有一个String
+ 键入p就可以出来（这是一个类似Live Template的效果，在语言中不是必须的，但是就顺便讲下）
...
请注意，编辑完之后请**Ctrl+S**保存，这样MPS才会识别你的编辑结果。
...
然后你左边的Logical View会多出来一个自动生成的map_PrintlnSet，先不管它。
