...
后来我还做了一个语法糖，就是把 `__ + 1` 解糖成 `\lam p0 => p0 + 1`，
`f __ + 1` 解糖成 `\lam p0 => f p0 + 1` 的这个双下划线语法（具体的规则还挺复杂的）。
...
UT Austin 的人在看到了我的在 HoTT-Arend 里的评论后对我表示了兴趣，打了个 zoom 电话给我，
问了些莫名其妙的问题，比如 HoTT 在 CH 同构意义上是和什么逻辑对偶的，Arend 的 `iso`
是怎么实现的，做 Arend 的 picture 是怎样的，我是怎么知道 Arend 的。
我的回答都很简单——普通的高阶逻辑加 Axiom，特殊的 reduction rule，formalize all the math，
以及 "网上看到的"。
